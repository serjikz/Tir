...
void Enemy::update(float dt) {
	_x += _vecMove.x * dt;
	_y += _vecMove.y * dt;
	// ѕри обновлении учитыаем трение среды
	...
	// ƒаем врем€ разлететьс€, чтобы тексутры не "слиплись"
...
void Enemy::checkScreenBounce() {
	// ѕровер€ем столкновении с границами экрана
...
bool Enemy::isIntersect(Enemy::HardPtr anotherEnemy) {
	// рассто€ние между центрами текстур
...
void Enemy::bounceWith(Enemy::HardPtr anotherEnemy) {
	// даем врем€ разлететьс€, чтобы тексутры не "слиплись"
	if (_isBounced && _enemyLastBounced == anotherEnemy) {
		return;
	}
	_t = 0.f;
	_isBounced = true;
	// вектор взаимодействи€
	FPoint interactionVec = anotherEnemy->getCenterPos() - this->getCenterPos();
